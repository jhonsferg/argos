...
// from an argos YAML config file, under the "mongo" key. Pointer fields
...
// WithYAMLConfig applies the "mongo" section of cfg.Integrations (as
...
// YAML:
//
//	options.Client().SetMonitor(argosmongo.NewMonitor(argosmongo.WithYAMLConfig(cfg)))
func WithYAMLConfig(cfg core.Config) Option {
	return func(c *config) {
		var y yamlConfig
		ok, err := core.DecodeIntegrationConfig(cfg, "mongo", &y)
		if err != nil || !ok {
			return
		}
		if y.QueryText != nil {
