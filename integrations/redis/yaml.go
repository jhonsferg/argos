...
// from an argos YAML config file, under the "redis" key. Pointer fields
...
// WithYAMLConfig applies the "redis" section of cfg.Integrations (as
...
// YAML:
//
//	client.AddHook(argosredis.NewHook(argosredis.WithYAMLConfig(cfg)))
func WithYAMLConfig(cfg core.Config) Option {
	return func(c *config) {
		var y yamlConfig
		ok, err := core.DecodeIntegrationConfig(cfg, "redis", &y)
		if err != nil || !ok {
			return
		}
		if y.QueryText != nil {
