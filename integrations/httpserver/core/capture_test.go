...
func TestMiddleware_CaptureBodyOnError_CapturesOnFailure(t *testing.T) {
	exp := setTracer(t)
	...
	req := httptest.NewRequest(http.MethodPost, "/boom", strings.NewReader(`{"user_id":42}`))
...
func TestMiddleware_CaptureBodyOnError_NoCaptureOnSuccess(t *testing.T) {
	exp := setTracer(t)
	instr := New(WithCaptureBodyOnError(1024))
	...
	req := httptest.NewRequest(http.MethodPost, "/ok", strings.NewReader(`{"user_id":42}`))
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)

	if rec.Body.String() != `{"ok":true}` {
		t.Fatalf("response body reaching the client = %q, want unaffected by capture", rec.Body.String())
	}
...
func TestMiddleware_CaptureBodyOnError_TruncatesAtMax(t *testing.T) {
	exp := setTracer(t)
	instr := New(WithCaptureBodyOnError(4))
	...
	req := httptest.NewRequest(http.MethodPost, "/boom", strings.NewReader(`abcdefgh`))
	handler.ServeHTTP(httptest.NewRecorder(), req)

	spans := exp.GetSpans()
	for _, kv := range spans[0].Attributes {
		if string(kv.Key) == "argos.http.request.body" && len(kv.Value.AsString()) > 4 {
			t.Errorf("captured request body = %q (%d bytes), want truncated at 4", kv.Value.AsString(), len(kv.Value.AsString()))
		}
	}
}

func TestMiddleware_WithCapture_HeadersExcludeAndMask(t *testing.T) {
	exp := setTracer(t)
	instr := New(WithCapture(capture.HTTPRules{
...
func TestMiddleware_WithCapture_HeadersOnErrorOnly(t *testing.T) {
	exp := setTracer(t)
	instr := New(WithCapture(capture.HTTPRules{
...
func TestMiddleware_NoCaptureConfigured_NoOverhead(t *testing.T) {
	exp := setTracer(t)
	instr := New() // no WithCaptureBodyOnError

	handler := instr.Middleware(staticRoute("/boom"))(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusInternalServerError)
	}))

	req := httptest.NewRequest(http.MethodPost, "/boom", strings.NewReader(`sensitive`))
	handler.ServeHTTP(httptest.NewRecorder(), req)
